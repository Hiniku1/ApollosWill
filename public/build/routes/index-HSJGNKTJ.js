...
function Index() {
  ...
  connection.query("INSERT INTO Anime (id, slug, synopsis, titles, abbreviated_titles, start_date, end_date, rating_rank, subtype, status, poster_image, cover_image, episode_count) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [data.findAnimeById.id, data.findAnimeById.slug, data.findAnimeById.description, data.findAnimeById.titles.localized.join(","), data.findAnimeById.titles.alternatives.join(","), data.findAnimeById.startDate, data.findAnimeById.endDate, data.findAnimeById.averageRatingRank, data.findAnimeById.subtype, data.findAnimeById.status, data.findAnimeById.posterImage.original.url, data.findAnimeById.bannerImage.original.url, data.findAnimeById.episodeCount], function(error3, results) {
    if (error3) {
...
//# sourceMappingURL=/build/routes/index-HSJGNKTJ.js.map
