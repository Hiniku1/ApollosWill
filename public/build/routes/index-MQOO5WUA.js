...
var require_LocalDataFilePacket = __commonJS({
  "node_modules/mysql/lib/protocol/packets/LocalDataFilePacket.js"(exports, module) {
    ...
    function LocalDataFilePacket(data2) {
      this.data = data2;
    }
...
var require_RowDataPacket = __commonJS({
  "node_modules/mysql/lib/protocol/packets/RowDataPacket.js"(exports, module) {
    ...
    function parse3(parser, fieldPackets, typeCast2, nestTables, connection2) {
      var self2 = this;
      var next = function() {
        return self2._typeCast(fieldPacket, parser, connection2.config.timezone, connection2.config.supportBigNumbers, connection2.config.bigNumberStrings, connection2.config.dateStrings);
      };
      for (var i = 0; i < fieldPackets.length; i++) {
        var fieldPacket = fieldPackets[i];
        var value;
        if (typeof typeCast2 === "function") {
          value = typeCast2.apply(connection2, [new Field({ packet: fieldPacket, parser }), next]);
        } else {
          value = typeCast2 ? this._typeCast(fieldPacket, parser, connection2.config.timezone, connection2.config.supportBigNumbers, connection2.config.bigNumberStrings, connection2.config.dateStrings) : fieldPacket.charsetNr === Charsets.BINARY ? parser.parseLengthCodedBuffer() : parser.parseLengthCodedString();
        }
...
var require_Auth = __commonJS({
  "node_modules/mysql/lib/protocol/Auth.js"(exports) {
    ...
    var Auth = exports;
    function auth(name, data2, options) {
      options = options || {};
      switch (name) {
        case "mysql_native_password":
          return Auth.token(options.password, data2.slice(0, 20));
        default:
          return void 0;
      }
    }
...
var require_ChangeUser = __commonJS({
  "node_modules/mysql/lib/protocol/sequences/ChangeUser.js"(exports, module) {
    ...
    ChangeUser.prototype["AuthSwitchRequestPacket"] = function(packet) {
      ...
      var data2 = Auth.auth(name, packet.authMethodData, {
...
var require_stream_transform = __commonJS({
  "node_modules/mysql/node_modules/readable-stream/lib/_stream_transform.js"(exports, module) {
    ...
    function done2(stream, er, data2) {
      if (er)
...
var require_Query = __commonJS({
  "node_modules/mysql/lib/protocol/sequences/Query.js"(exports, module) {
    ...
    Query.prototype["RowDataPacket"] = function(packet, parser, connection2) {
      packet.parse(parser, this._resultSet.fieldPackets, this.typeCast, this.nestTables, connection2);
      if (this._callback) {
    ...
    Query.prototype._sendLocalDataFile = function(path) {
      ...
      localStream.on("data", function(data2) {
        self2.emit("packet", new Packets.LocalDataFilePacket(data2));
      });
...
var require_Protocol = __commonJS({
  "node_modules/mysql/lib/protocol/Protocol.js"(exports, module) {
    ...
    Protocol.prototype._debugPacket = function(incoming, packet) {
      var connection2 = this._connection;
      var direction = incoming ? "<--" : "-->";
      var packetName = packet.constructor.name;
      var threadId = connection2 && connection2.threadId !== null ? " (" + connection2.threadId + ")" : "";
  ...
  "node_modules/mysql/lib/Connection.js"(exports, module) {
    ...
    Connection.prototype.connect = function connect(options, callback) {
      ...
      if (!this._connectCalled) {
        ...
        this._protocol.on("data", function(data2) {
          connection2._socket.write(data2);
        });
        this._socket.on("data", wrapToDomain(connection2, function(data2) {
          connection2._protocol.write(data2);
        }));
        this._protocol.on("end", function() {
          connection2._socket.end();
        });
        this._socket.on("end", wrapToDomain(connection2, function() {
          connection2._protocol.end();
        }));
    ...
    if (tls.TLSSocket) {
      Connection.prototype._startTLS = function _startTLS(onSecure) {
        ...
        createSecureContext(this.config, function(err, secureContext) {
          ...
          connection2._socket.removeAllListeners("data");
          connection2._protocol.removeAllListeners("data");
          var rejectUnauthorized = connection2.config.ssl.rejectUnauthorized;
          var secureEstablished = false;
          var secureSocket = new tls.TLSSocket(connection2._socket, {
            rejectUnauthorized,
            requestCert: true,
            secureContext,
            isServer: false
          ...
          secureSocket.on("_tlsError", function(err2) {
            if (secureEstablished) {
              connection2._handleNetworkError(err2);
            } else {
              onSecure(err2);
            }
          });
          secureSocket.pipe(connection2._protocol);
          connection2._protocol.on("data", function(data2) {
            secureSocket.write(data2);
          });
      ...
      Connection.prototype._startTLS = function _startTLS(onSecure) {
        ...
        this._socket.on("data", function(data2) {
          securePair.encrypted.write(data2);
        });
        securePair.cleartext.pipe(this._protocol);
        this._protocol.on("data", function(data2) {
          securePair.cleartext.write(data2);
        });
  ...
  "node_modules/mysql/lib/Pool.js"(exports, module) {
    ...
    Pool.prototype.getConnection = function(cb) {
      ...
      if (this._freeConnections.length > 0) {
        ...
        this.acquireConnection(connection2, cb);
        return;
      }
      if (this.config.connectionLimit === 0 || this._allConnections.length < this.config.connectionLimit) {
        connection2 = new PoolConnection(this, { config: this.config.newConnectionConfig() });
        this._acquiringConnections.push(connection2);
        this._allConnections.push(connection2);
        connection2.connect({ timeout: this.config.acquireTimeout }, function onConnect(err2) {
          spliceConnection(pool._acquiringConnections, connection2);
          if (pool._closed) {
            err2 = new Error("Pool is closed.");
            err2.code = "POOL_CLOSED";
          }
          if (err2) {
            pool._purgeConnection(connection2);
            cb(err2);
            return;
          }
          pool.emit("connection", connection2);
          pool.emit("acquire", connection2);
          cb(null, connection2);
        });
        return;
      }
    ...
    Pool.prototype.acquireConnection = function acquireConnection(connection2, cb) {
      if (connection2._pool !== this) {
        throw new Error("Connection acquired from wrong pool.");
      }
      var changeUser = this._needsChangeUser(connection2);
      var pool = this;
      this._acquiringConnections.push(connection2);
      ...
      if (changeUser) {
        connection2.config = this.config.newConnectionConfig();
        connection2.changeUser({ timeout: this.config.acquireTimeout }, onOperationComplete);
      } else {
        connection2.ping({ timeout: this.config.acquireTimeout }, onOperationComplete);
      }
    };
    Pool.prototype.releaseConnection = function releaseConnection(connection2) {
      if (this._acquiringConnections.indexOf(connection2) !== -1) {
        return;
      }
      if (connection2._pool) {
        ...
        if (this._freeConnections.indexOf(connection2) !== -1) {
    ...
    Pool.prototype._needsChangeUser = function _needsChangeUser(connection2) {
      var connConfig = connection2.config;
    ...
    Pool.prototype._purgeConnection = function _purgeConnection(connection2, callback) {
      ...
      if (connection2.state !== "disconnected" && !connection2._protocol._quitSequence) {
        connection2._realEnd(cb);
        return;
      }
      process.nextTick(cb);
    };
    Pool.prototype._removeConnection = function(connection2) {
    ...
    function spliceConnection(array, connection2) {
      var index;
      if ((index = array.indexOf(connection2)) !== -1) {
...
var require_PoolNamespace = __commonJS({
  "node_modules/mysql/lib/PoolNamespace.js"(exports, module) {
    ...
    PoolNamespace.prototype.getConnection = function(cb) {
      ...
      cluster._getConnection(clusterNode, function(err2, connection2) {
...
var require_PoolCluster = __commonJS({
  "node_modules/mysql/lib/PoolCluster.js"(exports, module) {
    ...
    PoolCluster.prototype._getConnection = function(node, cb) {
      var self2 = this;
      node.pool.getConnection(function(err, connection2) {
        if (err) {
...
connection.query("INSERT INTO Anime (id, slug, synopsis, titles, abbreviated_titles, start_date, end_date, rating_rank, subtype, status, poster_image, cover_image, episode_count) VALUES (?, ?)", [data.id, data.slug, data.description, data.titles.localized, data.startDate, data.endDate, data.averageRatingRank, data.subtype, data.status, data.posterImage.original.url, data.bannerImage.original.url, data.episodeCount], function(error2, results) {
  if (error2) {
    console.log("idk mate");
  }
});
function Index() {
...
//# sourceMappingURL=/build/routes/index-MQOO5WUA.js.map
